...
✨ Features
- Add **Grok (xAI) provider support**: full integration using persistent ACP (long-lived sessions via Grok CLI over stdio), including context window bar (n.n% nnnk / 500k Context using input/prompt tokens from usage + model catalog limits), live billing/credits via /usage, OAuth (`grok login`) and API key auth, permission modes, session history reader, model selection (grok-4.5, grok-build etc.), i18n across locales, and /context dialog synthesis. Mirrors Claude/Codex daemon patterns for multi-turn UX. (by @toxeh)
